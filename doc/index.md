---
layout: home
title: Pixiv 书源 - 最好的 Pixiv 第三方小说阅读器
description: 用开源阅读 + Pixiv 书源，像看网文一样阅读 Pixiv 小说。支持搜索、收藏、追更、评论、直连评论等功能。

head:
  - - meta
    - name: keywords
      content: Pixiv github, Pixiv 第三方, Pixiv 第三方 app, Pixiv 阅读器, Pixiv 小说, Pixiv 小説, Linpx, 兽人小说, FurryNovel, 书源, Pixiv 书源, Linpx 书源, FurryNovel 书源, PixivSource, Pixiv BookSource, 兽人小说 书源, 开源阅读, 开源阅读 书源, 开源阅读书源
  - - meta
    - property: og:title
      content: Pixiv 书源 - 最好的 Pixiv 第三方小说阅读器
  - - meta
    - property: og:description
      content: 用开源阅读 + Pixiv 书源，像看网文一样阅读 Pixiv 小说。支持搜索、收藏、追更、评论、直连、评论等功能。
  - - meta
    - property: og:image
      content: https://pixivsource.pages.dev/favicon-180x180.png
  - - meta
    - name: twitter:card
      content: summary
  - - meta
    - name: twitter:title
      content: Pixiv 书源 - 最好的 Pixiv 第三方小说阅读器
  - - meta
    - name: twitter:description
      content: 用开源阅读 Legado + Pixiv 书源，像看网文一样看 Pixiv 小说。支持搜索、收藏、追更、评论等功能。


hero:
  name: 开源阅读 + Pixiv 书源
  text: 最好的 Pixiv 小说软件
  tagline: <br>你值得拥有，更好的小说阅读体验<br>⬇️ 点击【✨ 臻享阅读】改善你的阅读体验吧

  image:
    src: "/PixivSource.png"
    alt: Pixiv Source Logo
  actions:
    - theme: brand
      text: ⬇️ 下载 阅读 Sigma
      link: DownloadLegado

    - theme: brand
      text: 🚀 一键导入
      link: Import
      
    - theme: alt
      text: ✨ 臻享阅读
      link: BetterExperience
      
    - theme: alt
      text: ⚡️ 快速开始
      link: QuickStart

    - theme: brand
      text: 🚀 导入 Pixiv 书源
      link: "legado://import/importonline?src=https://cdn.jsdelivr.net/gh/DowneyRem/PixivSource@main/pixiv.json"
      
    - theme: brand
      text: 🚀 导入 Linpx 书源
      link: "legado://import/importonline?src=https://cdn.jsdelivr.net/gh/DowneyRem/PixivSource@main/linpx.json"
      
    - theme: brand
      text: 🚀 导入 订阅源
      link: "legado://import/importonline?src=https://cdn.jsdelivr.net/gh/DowneyRem/PixivSource@main/btsrk.json"
      
    - theme: alt
      text: 📘 Pixiv 书源手册
      link: Pixiv
      
    - theme: alt
      text: 📙 Linpx 书源手册
      link: Linpx    

#    - theme: alt
#      text: 🏠 项目
#      link: https://github.com/DowneyRem/PixivSource
    
    - theme: alt
      text: ✈️ 频道 
      link: https://t.me/PixivSource
      
    - theme: alt
      text: 💖 打赏
      link: Sponsor
      
#    - theme: alt
#      text: 📜 更新日志
#      link: UpdateLog


features:
  - icon: 📖
    title: 全面支持阅读功能
    details: 🔍搜索小说 ⭐️发现小说<br>🔗添加网址 🌐订阅源
    link: AddNovels
    
  - icon: 📚
    title: 阅读体验全面升级
    details: 搜索：📄单篇📚系列👤作者<br>查看：🖼插图✅投票💬评论
    link: Pixiv#QuickJump
    
  - icon: ⭐️
    title: 互动功能一应俱全
    details: ❤️收藏📃追更⭐️关注<br>✅投票💬评论🚫屏蔽
    link: Pixiv#InteractiveFunction
    
  - icon: ✈️
    title: 直连功能无需代理
    details: 🅿️ 登录账号，开启直连<br>✈️ 无需代理，直接访问
    link: BetterExperience#IPDirect
---
<HomeUnderline />
<FriendLinkFooterHome />


> [!WARNING]
>
> ⚠️ **你正在 GitHub 上浏览此文档， Github 文档可能不完整
> [网页版](https://pixivsource.pages.dev)
> 内容更全面，排版更精美**